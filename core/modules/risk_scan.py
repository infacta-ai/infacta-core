def scan_risks(state: dict) -> dict:
    ...
    if "штраф" in text_lower or "penalty" in text_lower:
        risks.append("Виявлено пункт про штраф / penalty clause")

    if "односторон" in text_lower or "unilateral" in text_lower:
        risks.append("Можливі односторонні зобов’язання")

    if "без відповідальності" in text_lower or "no liability" in text_lower:
        risks.append("Можливе обмеження відповідальності")

    if "автоматичне продовження" in text_lower or "automatic renewal" in text_lower or "auto renewal" in text_lower:
        risks.append("Виявлено автоматичне продовження договору")

    if "non-refundable" in text_lower:
        risks.append("Виявлено умову non-refundable")
